...
- (void)setDataSource:(id<DOPDropDownMenuDataSource>)dataSource {
    ...
    lineLayer.frame = CGRectMake(self.layer.frame.size.width/2 -1, 8, 1, self.layer.frame.size.height-16);
...
- (void)menuTapped:(UITapGestureRecognizer *)paramSender {
    ...
    if (tapIndex == 1) {
        ...
        [self.delegate filterButtonTapAction];
        return;
        
    }
    if (!_show) {
        /**
         *  坐标系转换 计算绝对值 忽略ScorllView的contentOffset
         */
        CGPoint headerPoint  = self.superview.origin;
        UIScrollView *sView =(UIScrollView *) self.customSuperView;
        CGPoint contentOffset = sView.contentOffset;
        CGFloat yPoint =  fabs(contentOffset.y - headerPoint.y);
        self.customOrigin = CGPointMake(0, yPoint);
        
        
        self.tableView.origin = self.customOrigin;
        self.backGroundView.origin = CGPointMake(self.customOrigin.x, self.customOrigin.y +self.height);
    }
...
- (void)animateBackGroundView:(UIView *)view show:(BOOL)show complete:(void(^)())complete {
    if (show) {
        [self.customSuperSuperView addSubview:view];
...
- (void)animateTableView:(UITableView *)tableView show:(BOOL)show complete:(void(^)())complete {
    if (show) {
        tableView.frame = CGRectMake(self.customOrigin.x, self.customOrigin.y + self.frame.size.height, self.frame.size.width, 0);
        [self.customSuperSuperView addSubview:tableView];
        ...
        [UIView animateWithDuration:0.2 animations:^{
            _tableView.frame = CGRectMake(self.customOrigin.x, self.customOrigin.y + self.frame.size.height, self.frame.size.width, tableViewHeight);
        }];

    } else {
        [UIView animateWithDuration:0.2 animations:^{
//            _tableView.frame = CGRectMake(self.tableView.xPosition, self.tableView.yPosition + 50, self.tableView.width, 0);
            _tableView.height = 0;
        } completion:^(BOOL finished) {
