...
- (void)commonInit {
    ...
    _navView.backgroundColor = [UIColor colorWithHexString:@"#190d31"];
    
    _nameLabel = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/4, 20, SCREEN_WIDTH/2, 44)];
    _nameLabel.text = @"用户主页";
...
- (void)adjustItemViewContentOffset:(UIScrollView *)itemView atIndex:(NSInteger)index fromLastItemView:(UIScrollView *)lastItemView lastIndex:(NSInteger)lastIndex {
    ...
    if (lastIndex == index) {
        ...
        if (_isNavHidden) {
            ...
            initContentOffset = CGPointMake(0, - 0);
        }
#endif
...
//            NSLog(@"topMarginOffset=%f, newOffsetY=%f,Bar.st_top=%f",topMarginOffset, newOffsetY, _swipeHeaderBar.st_top);
            ...
            if (newOffsetY < topMarginOffset) {
                ...
                if (newOffsetY>80) {
                    ...
                    _nameLabel.text = self.navTitleStr;
                    [self.parentViewController.view addSubview:_navView];
                    CGFloat offsetToAlpa = (newOffsetY-80.0)/64.0;
                    if (offsetToAlpa>1.0) {
                            
                        offsetToAlpa = 1.0;
                    }
                    _navView.alpha = offsetToAlpa;
...
- (void)headerViewDidFrameChanged:(STHeaderView *)headerView {
    ...
    if (_isNavHidden) {
        ...
        if (offsetY >= 80) {
            ...
            _nameLabel.text = self.navTitleStr;
            [self.parentViewController.view addSubview:_navView];
            CGFloat offsetToAlpa = (offsetY-80.0)/64.0;
            if (offsetToAlpa>1.0) {
