...
@interface ChatViewController ()<UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, SRRefreshDelegate, IChatManagerDelegate, DXChatBarMoreViewDelegate, DXMessageToolBarDelegate, LocationViewDelegate, IDeviceManagerDelegate,SatisfactionDelegate,UIAlertViewDelegate,EMCDDeviceManagerDelegate>
{
...
//        _conversation = [[EaseMob sharedInstance].chatManager conversationForChatter:chatter isGroup:_isChatGroup];
        ...
        [_conversation markAllMessagesAsRead:YES];
        if (!_conversation) {
            [HTUIHelper addHUDToWindowWithString:@"登录失败,请稍后再试,或退出App重试" hideDelay:2];
        }
    }
...
//    self.title = NSLocalizedString(@"title.customer", @"Customer");
    ...
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(closeView) name:@"kHuanXinLoginFromOtherDevice" object:nil];


}
- (void)closeView{
    [HTUIHelper addHUDToWindowWithString:@"该账户已从其他设备登录，请重新进入客服界面" hideDelay:2];
...
- (void)viewWillDisappear:(BOOL)animated
{
     ...
     *  去除小红点
     */
    [RJAppManager sharedInstance].isNewKeFuMessage = NO;
}
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if (cell == nil) {
                ...
                if (model.isSender) {
                    ...
                    NSString *userIconUrlStr = accountModel.avatar;
                    
                    if (userIconUrlStr.length==0) {
                        userIconUrlStr = @"Icon_for_share";
                    }
                    
                    [cell.headImageView sd_setImageWithURL:[NSURL URLWithString:userIconUrlStr] placeholderImage:[UIImage imageNamed:@"Icon_for_share"]];
                }
                else{
                    cell.headImageView.image = [UIImage imageNamed:@"Icon_for_share"];
                }
            }
...
- (void)moreViewTakePicAction:(DXChatBarMoreView *)moreView
{
    ...
    self.imagePicker.mediaTypes = @[(NSString *)kUTTypeImage];
//    if ([self checkOuthStatusForCameralib]) {
//        
//    }
    [self presentViewController:self.imagePicker animated:YES completion:NULL];
...
- (BOOL)checkOuthStatusForPhotolib {
    ...
    if (status == ALAuthorizationStatusNotDetermined) {
        ...
        [HTUIHelper alertMessage:@"相册无使用权限,请在设置中更改"];
        return NO;
    }
...
- (BOOL)checkOuthStatusForCameralib{
    AVAuthorizationStatus authStatus =  [AVCaptureDevice authorizationStatusForMediaType:AVMediaTypeVideo];
    if (authStatus == AVAuthorizationStatusRestricted || authStatus ==AVAuthorizationStatusDenied)
    {
        [HTUIHelper alertMessage:@"相机无使用权限，请在设置中更改"];
        return NO;
    }
    return YES;
}
...
//        [self showHint:NSLocalizedString(@"message.noMessage", @"no messages")];
    ...
    UIAlertView * alertView = [[UIAlertView alloc]initWithTitle:@"" message:@"确定要删除吗？" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil];
    [alertView show];
...
- (NSDictionary*)getUserInfoAttribute
{
    ...
    [visitor setObject:@"user" forKey:@"trueName"];
    [visitor setObject:@"10000" forKey:@"qq"];
    [visitor setObject:@"13512345678" forKey:@"phone"];
    [visitor setObject:@"环信" forKey:@"companyName"];
    NSString *nickname = [RJAccountManager sharedInstance].account.nickname.length?[RJAccountManager sharedInstance].account.nickname:[RJAccountManager sharedInstance].account.username.length?[RJAccountManager sharedInstance].account.username:@"无用户名";
