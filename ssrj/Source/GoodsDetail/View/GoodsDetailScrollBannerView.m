...
@interface GoodsDetailScrollBannerView()<UIScrollViewDelegate>{
...
@end
@implementation GoodsDetailScrollBannerView
-(void)awakeFromNib{
    [super awakeFromNib];
...
- (void)imageTapGesture:(UITapGestureRecognizer *)sender{
    ...
    if (model.videoPath.length) {
        ...
        [self.indicatorView removeFromSuperview];
        [sender.view addSubview:self.indicatorView];
        self.indicatorView.center = CGPointMake(sender.view.width/2, sender.view.width/2);
