...
@interface RJFilterPriceViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation RJFilterPriceViewController
- (void)viewDidLoad{
    [super viewDidLoad];
    [self addSaveButton];
    [self addCloseButton];

    [self setTitle:@"价格" tappable:NO];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    RJFilterPriceModel *model = self.dataArray[indexPath.row];
...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    [MobClick beginLogPageView:@"筛选价格界面"];
    [TalkingData trackPageBegin:@"筛选价格界面"];

    
}
- (void)viewWillAppear:(BOOL)animated{
    [super viewWillAppear:animated];
    [MobClick endLogPageView:@"筛选价格界面"];
    [TalkingData trackPageEnd:@"筛选价格界面"];
