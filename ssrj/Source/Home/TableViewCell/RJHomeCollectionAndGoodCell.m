...
- (void)setModel:(RJHomeItemTypeTwoModel *)model{
    ...
    [self.collectionImageView sd_setImageWithURL:[NSURL URLWithString:model.path] placeholderImage:GetImage(@"default_1x1") completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        [weakSelf.collectionImageView sd_setImageWithURL:[NSURL URLWithString:model.path] placeholderImage:GetImage(@"default_1x1") completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            if (!error) {
    ...
    self.putIntoThemeButton.trackingId = [NSString stringWithFormat:@"%@&RJHomeCollectionAndGoodCell&putIntoThemeButton&id=%@",vcName,model.id.stringValue];
    self.likeButton.trackingId = [NSString stringWithFormat:@"%@&RJHomeCollectionAndGoodCell&likeButton&id=%@",vcName,model.id.stringValue];
    ...
    if (dataArr.count >= 1) {
        ...
        self.goodOneImageView.goodsModel = model;
         ...
         *  统计ID
         */
        self.goodViewOne.trackingId = [NSString stringWithFormat:@"%@&%@&goodViewOne&id=%@",vcName,NSStringFromClass(self.class),model.goodId];
        ...
        if (dataArr.count>=2) {
            ...
            self.goodTwoImageView.goodsModel = model;
            ...
            self.goodViewTwo.trackingId = [NSString stringWithFormat:@"%@&%@&goodViewTwo&id=%@",vcName,NSStringFromClass(self.class),model.goodId];
    ...
    for (int i = 0; i<self.model.themeTagList.count; i++) {
         ...
         *  统计ID
         */
        button.trackingId = [NSString stringWithFormat:@"%@&%@&TagButton&id=%@&tagId=%@",vcName,NSStringFromClass(self.class),self.model.id.stringValue,itemModel.themeItemId.stringValue];
        [button sizeToFit];
