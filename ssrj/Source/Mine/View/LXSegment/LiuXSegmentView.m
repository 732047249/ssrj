...
#define RGBColor [UIColor colorWithRed:255/255.0 green:92/255.0 blue:79/255.0 alpha:1]
...
-(instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titleArray clickBlick:(btnClickBlock)block{
    ...
    if (self) {
        ...
        if (titleArray.count<MAX_TitleNumInWindow+1) {
            _btn_w=SCREEN_WIDTH/titleArray.count;
        }else{
            _btn_w=SCREEN_WIDTH/MAX_TitleNumInWindow;
        }
        _titles=titleArray;
        _defaultIndex=1;
...
//        _titleSelectColor=RGBColor;
        
#pragma mark -- 导航栏选中的颜色
        _titleSelectColor=[UIColor colorWithRed:67/255.0 green:0/255.0 blue:212/255.0 alpha:1];
        
        _bgScrollView=[[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, self.frame.size.height)];
