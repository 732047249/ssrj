//
//  UsedHongBaoTableViewController.m
//  ssrj
//
...
@interface UsedHongBaoTableViewController ()<UIGestureRecognizerDelegate>
...
@implementation UsedHongBaoTableViewController

-(void)viewWillAppear:(BOOL)animated{
    [super viewWillAppear:animated];
    
    [MobClick beginLogPageView:@"已使用的红包"];
    [TalkingData trackPageBegin:@"已使用的红包"];
...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    [MobClick endLogPageView:@"已使用的红包"];
    [TalkingData trackPageEnd:@"已使用的红包"];
...
//    NSString *urlStr = [NSString stringWithFormat:@"/b82/api/v2/user/findcoupon?userId=%@", @"2377"];
    ...
    [[ZHNetworkManager sharedInstance] getWithRequestInfoWithoutModel:requestInfo success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if ([responseObject objectForKey:@"state"]) {
                ...
                if (state.intValue == 0) {
                    ...
                    NSArray *UsedCouponArr = [data objectForKey:@"UsedCoupon"];
                    
                    [weakSelf.dataArr removeAllObjects];
                
                    NSMutableArray *arrayModels = [NSMutableArray array];
                    
                    for (NSDictionary *tempDic in UsedCouponArr) {
                        
                        NSError *error = nil;
                        
                        CouponModel *model = [[CouponModel alloc] initWithDictionary:tempDic error:&error];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    couponCell.couponStateImageView.image = [UIImage imageNamed:@"used_icon"];
    ...
    [couponCell.useRuleButton addTarget:self action:@selector(unUseRuleButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)unUseRuleButtonClicked:(UIButton *)sender {
