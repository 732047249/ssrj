...
static NSString * const SearchListUrl = @"/b180/api/v1/goodsinfor/goods";
@interface SMSearchGoodsController ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate,UITextFieldDelegate>
...
@implementation SMSearchGoodsController
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (self.isFromInformation) {
        [MobClick beginLogPageView:@"创建资讯-单品名称列表页面"];
        [TalkingData trackPageBegin:@"创建资讯-单品名称列表页面"];
    }else {
        [MobClick beginLogPageView:@"创建搭配-单品名称列表页面"];
        [TalkingData trackPageBegin:@"创建搭配-单品名称列表页面"];
    }
}
- (void)viewWillDisappear:(BOOL)animated {
    [super viewWillDisappear:animated];
    if (self.isFromInformation) {
        [MobClick endLogPageView:@"创建资讯-单品名称列表页面"];
        [TalkingData trackPageEnd:@"创建资讯-单品名称列表页面"];
    }else {
        [MobClick endLogPageView:@"创建搭配-单品名称列表页面"];
        [TalkingData trackPageEnd:@"创建搭配-单品名称列表页面"];
    }
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    //创建资讯： 进入单品详情
    if (self.isFromInformation) {
        HHInforGoodsSearchDetailController *detail = [[HHInforGoodsSearchDetailController alloc] init];
        detail.searchName = selectString;
        [self.navigationController pushViewController:detail animated:YES];
    }
    //创建搭配： 进入素材或单品详情
    else {
        SMSearchDetailController *detail = [[SMSearchDetailController alloc]init];
        detail.isFromAllGoods = self.isFromAllGoods;
        detail.searchName = selectString;
        [self.navigationController pushViewController:detail animated:YES];
...
//    for (NSMutableAttributedString *att in arr) {
//        
////        NSLog(@"%@",att.string);
//    }
